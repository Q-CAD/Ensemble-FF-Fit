...
generate_vaspflux -pd md_images/ -vy inputs/vdW_single_point.yml -vs inputs/perlmutter_vasp_gpu.sh -ve vasp_gam -a m2113_g -apn 120

# Step 2: Run the main Flux submission workflow; can specify arguments
echo "Running the main Flux submission workflow..."
srun -c 32 --external-launcher --mpi=pmi2 --gpu-bind=closest flux start matsemble_vaspflux -pd md_images/
